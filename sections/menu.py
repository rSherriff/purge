        ...
        for i in range(0, 3):
            if os.path.isfile(menu_panel_xp_file + str(i+1) + '.xp'):
                ...
                self.animated_tiles.append(
                    np.full((self.width, self.height), fill_value=tile_types.blank, order="F"))

                for h in range(0, self.height):
                    for w in range(0, self.width):
                        self.animated_tiles[i][w,
                                               h]['graphic'] = xp_data['layer_data'][0]['cells'][w][h]
    ...
    def render(self, console):
        ...
        if len(self.animated_tiles[self.selected_tiles]) > 0:
            if self.invisible == False:
                console.tiles_rgb[self.x: self.x + self.width, self.y: self.y +
                                  self.height] = self.animated_tiles[self.selected_tiles]["graphic"]
